...
def askopenfile1():
    ...
    print(path)
    im = im.resize((200, 200), Image.LANCZOS) #resizing the fetched image
...
def askopenfile2():
    path = filedialog.askopenfilename(
        filetypes=[("Image File", '.jpg'), ("Image File", '.png'), ("Image File", '.tiff')]
    )
    ...
    if os.path.isdir(path):  # If the path is a directory instead of a file
        print("Error: Selected path is a directory, not a file.")
        return

    try:
        loc2.delete(0, 'end')  # Clear the entry box
        loc2.insert(END, path)  # Insert the selected file path

        im = Image.open(path)  # Open the selected image
        im = im.resize((200, 200), Image.LANCZOS)
        tkimage = ImageTk.PhotoImage(im)

        myvar = Label(root, image=tkimage)
        myvar.image = tkimage
        myvar.grid(row=2, column=2, pady=5)

    except PermissionError:
        print("Error: Permission denied. Cannot open file.")
...
def askdirectory():
    global output_loc
    output_loc = filedialog.askdirectory()
    if not output_loc:  # If user cancels directory selection, set a default
